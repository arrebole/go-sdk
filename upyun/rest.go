...
type PutObjectConfig struct {
	...
	OnProgress func(fszie, offset, increase int64)
}
...
// 通过文件大小 自动获取分片大小, 按1% 与 1024 取整
func getPartSize(fsize int64) int64 {
	size := (fsize / 100 / 1024) * 1024
...
func (up *UpYun) getMultipartUploadProcess(config *PutObjectConfig, fsize int64) (*ResumeProcessResult, error) {
	...
	if !config.MultipartUploadCheckpoint {
		...
		return &ResumeProcessResult{
			...
			NextPartID:   0,
			NextPartSize: getPartSize(fsize),
	...
	// 兼容 GetResumeProcess 顺序上传
	// 顺序上传时只有 NextPartID 没有 Parts, 所以手动补全 Parts
	if resumeProcessResult.NextPartID > 0 && len(resumeProcessResult.Parts) == 0 {
		for i := int64(0); i < resumeProcessResult.NextPartSize; i++ {
			resumeProcessResult.Parts = append(resumeProcessResult.Parts, &DisorderPart{
				ID:   i,
				Size: getPartSize(fsize),
			})
		}
	}
